...
function buildGraph(people) {
    ...
    // ── Orbital Rings — Trump center, rings around ──
    positions[1] = { x: 0, y: 0, z: 0 };         // Trump at center
    positions[2] = { x: 0, y: -3.5, z: 0 };      // JD Vance, below

    // Cabinet ring around Trump
    const cabR = 14;
    cabinet.forEach((p, i) => {
        const angle = (i / cabinet.length) * Math.PI * 2 - Math.PI / 2;
        positions[p.id] = { x: Math.cos(angle) * cabR, y: Math.sin(angle) * cabR - 2, z: 0 };
    });

    // Administration ring further out
    const adminR = 20;
    admin.forEach((p, i) => {
        const angle = (i / admin.length) * Math.PI * 2 - Math.PI / 2;
        positions[p.id] = { x: Math.cos(angle) * adminR, y: Math.sin(angle) * adminR - 4, z: 0 };
    });

    // ── Judicial Branch (far left) ──
    const jx = -30;
    judicial.forEach((p, i) => {
        const row = Math.floor(i / 3);
        const col = i % 3;
        positions[p.id] = { x: jx + (col - 1) * 2.8, y: 8 - row * 3.2, z: -2 + col * 1.5 };
    });

    // ── Legislative Branch (far right) ──
    const lx = 30;
    legislative.forEach((p, i) => {
        const row = Math.floor(i / 3);
        const col = i % 3;
        positions[p.id] = { x: lx + (col - 1) * 2.8, y: 8 - row * 3.2, z: -2 + col * 1.5 };
    });

    // ── Tilt entire executive cluster 30° on X axis for true 3D depth ──
    const tilt = Math.PI / 6;
    const ct = Math.cos(tilt), st = Math.sin(tilt);
    [1, 2, ...cabinet.map(p => p.id), ...admin.map(p => p.id)].forEach(id => {
    ...
    people.forEach(p => {
        ...
        const pw = isTrump ? 2.6 : 1.8;
        const ph = isTrump ? 3.3 : 2.3;
        const borderRadius = 0.18;

        // ── Colored frame (rounded rect, behind photo) ──
        const frameGeo = roundedRectGeometry(pw + 0.16, ph + 0.16, borderRadius + 0.04, 4);
        ...
        frame.position.set(pos.x, pos.y, pos.z - 0.02);
        frame.userData = { isFrame: true };
        scene.add(frame);

        // ── Photo card (primary node) ──
        ...
        photoPlane.position.set(pos.x, pos.y, pos.z);
        photoPlane.userData = {
            id: p.id, name: p.name, role: p.role,
            branch: p.branch, bio: p.bio, party: p.party, color,
            isPhoto: true, cardW: pw, cardH: ph, cardR: borderRadius,
            frame: frame
        };
        frame.userData.parentNode = photoPlane;
        scene.add(photoPlane);
        nodes.push(photoPlane);
        nodeMap[p.id] = photoPlane;

        // Load texture and composite into rounded, cover-fit card
        const slug = photoSlug(p.name);
        const url = '/static/photos/' + slug + '.jpg';
        textureLoader.load(url,
            (texture) => {
                if (texture && texture.image) {
                    const cardTex = createRoundedCardTexture(texture.image, pw, ph, borderRadius);
                    photoPlane.material.map = cardTex;
                    photoPlane.material.color.set(0xffffff);
        ...
        labelSprite.position.set(pos.x, pos.y + (ph / 2) + (isTrump ? 0.8 : 0.65), pos.z);
        photoPlane.userData.label = labelSprite;
    ...
    // Cabinet inter-connections
    cabinet.forEach((a, i) => {
        cabinet.forEach((b, j) => {
            if (j > i && nodeMap[a.id] && nodeMap[b.id]) {
                const g = new THREE.BufferGeometry().setFromPoints([nodeMap[a.id].position.clone(), nodeMap[b.id].position.clone()]);
                scene.add(new THREE.Line(g, new THREE.LineBasicMaterial({ color: 0x1a2040, transparent: true, opacity: 0.15 })));
            }
        });
    });

    // ── Central orbit ring (tilted to match 30° cluster tilt) ──
    const orbitGeo = new THREE.TorusGeometry(11, 0.02, 16, 100);
    const orbitRing = new THREE.Mesh(orbitGeo, new THREE.MeshBasicMaterial({ color: 0x252d45, transparent: true, opacity: 0.3 }));
    orbitRing.position.copy(nodeMap[1].position);
...
// ── Raycaster for clicks (check frames + photo cards) ──
const raycaster = new THREE.Raycaster();
const clickTargets = () => {
    const all = [...nodes];
    nodes.forEach(n => {
        if (n.userData.frame) all.push(n.userData.frame);
    });
    return all;
};
...
renderer.domElement.addEventListener('click', (event) => {
    ...
    if (intersects.length > 0) {
        ...
        if (obj.userData.isFrame && obj.userData.parentNode) {
...
function animate() {
    ...
    // Billboard all photo cards and frames to face camera
    nodes.forEach(n => {
        n.lookAt(camera.position);
        if (n.userData.frame) n.userData.frame.lookAt(camera.position);
